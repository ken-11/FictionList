...
class FictioncontentSpider(scrapy.Spider):
    ...
    start_urls = ['http://top.17k.com/top/top100/01_subscribe/01_subscribe__top_100_pc.html']
    top_urls = [
    ...
    def parse(self, response):
        pattern = re.compile('<td width="30">([\s\S]*?)</td><td width="60">[\s\S]*?target="_blank">([\s\S]*?)</a></td><td><a class="red" href="([\s\S]*?)" title="([\s\S]*?)" target="_blank">[\s\S]*?</td>[\s\S]*?</td><td>([\s\S]*?)</td><td>[\s\S]*?" title="([\s\S]*?)"[\s\S]*?<td width="30">')
